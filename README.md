...
3. **Supabase** 통합 찾기 (또는 직접 https://dashboard.clerk.com/setup/supabase 접속)
4. **"Activate Supabase integration"** 버튼 클릭
5. 표시된 **Clerk domain** 복사 (예: `https://your-app-12.clerk.accounts.dev`)
...
- 📖 **상세 통합 가이드**: [`docs/CLERK_SUPABASE_INTEGRATION.md`](docs/CLERK_SUPABASE_INTEGRATION.md) 참고
- 📖 **Clerk 공식 문서**: [Clerk Supabase 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)

이제 Clerk와 Supabase가 완전히 통합되었습니다! 코드에서 `useClerkSupabaseClient()` (Client) 또는 `createClerkSupabaseClient()` (Server)를 사용하여 인증된 Supabase 클라이언트에 접근할 수 있습니다.
...
- 📖 **상세 가이드**: [`docs/CLERK_LOCALIZATION.md`](docs/CLERK_LOCALIZATION.md) - 한국어 로컬라이제이션 및 커스터마이징 방법

**기본 설정:**
- `lib/clerk/localization.ts`: 한국어 로컬라이제이션 설정 파일
- 기본 `koKR` 로컬라이제이션 사용
- 필요시 커스텀 메시지 추가 가능

### Supabase RLS (Row Level Security) 정책

프로젝트의 `users` 테이블에는 개발 편의를 위해 RLS가 비활성화되어 있습니다 (프로덕션에서는 활성화 권장).

**Clerk와 Supabase 통합 시 RLS 정책 작성 방법:**

- 📖 **상세 가이드**: [`docs/CLERK_SUPABASE_INTEGRATION.md`](docs/CLERK_SUPABASE_INTEGRATION.md) - RLS 정책 섹션 참고
- 📝 **예제 파일**: [`supabase/migrations/example_rls_policies.sql`](supabase/migrations/example_rls_policies.sql) - 실제 사용 가능한 RLS 정책 예제

**기본 원리:**
- Clerk 사용자 ID는 `auth.jwt()->>'sub'`로 접근 가능
- `user_id` 컬럼에 TEXT 타입으로 Clerk ID 저장
- RLS 정책에서 `(SELECT auth.jwt()->>'sub') = user_id`로 사용자 확인
...
│
├── lib/                   # 유틸리티 및 설정
│   ├── supabase/         # Supabase 클라이언트들 (공식 패턴 적용)
│   │   ├── clerk-client.ts      # Client Component용 (Clerk 인증)
│   │   ├── server.ts            # Server Component용 (Clerk 인증)
│   │   ├── server-public.ts     # Server Component용 (공개 데이터)
│   │   ├── service-role.ts      # 관리자용 (RLS 우회)
│   │   └── client.ts            # Client Component용 (공개 데이터)
...
│   │   ├── setup_storage.sql         # 스토리지 설정
│   │   └── example_rls_policies.sql  # RLS 정책 예제
...
- **`lib/supabase/`**: 환경별 Supabase 클라이언트 (매우 중요!)
  - `clerk-client.ts`: Client Component용 (Clerk 인증) - `useClerkSupabaseClient()`
  - `server.ts`: Server Component/Action용 (Clerk 인증) - `createClerkSupabaseClient()`
  - `server-public.ts`: Server Component용 (공개 데이터) - `createPublicSupabaseClient()`
...
- **`docs/CLERK_SUPABASE_INTEGRATION.md`**: Clerk + Supabase 통합 상세 가이드
- **`docs/SUPABASE_SETUP.md`**: Supabase 연결 및 사용 가이드
- **`supabase/migrations/example_rls_policies.sql`**: RLS 정책 작성 예제
